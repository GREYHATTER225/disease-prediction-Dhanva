...
from .models import patient , doctor , diseaseinfo , consultation ,rating_review , Appointment
...
try:
    ...
    model = None  # Fallback if model incompatible
...
def  consultationview(request,consultation_id):
    ...
    return render(request,'consultation/consultation_new.html', {"consultation":consultation_obj })
...
def doctor_dashboard(request):
    if request.user.is_authenticated and hasattr(request.user, 'doctor'):
        ...
        appointments = Appointment.objects.filter(doctor=doctor_obj)[:5]
        recent_consultations = consultation.objects.filter(doctor=doctor_obj)[:5]
        recent_reviews = rating_review.objects.filter(doctor=doctor_obj)[:5]
        total_patients = consultation.objects.filter(doctor=doctor_obj).count()
        total_appointments = appointments.count()
        doctor_rating = doctor_obj.rating
...
def book_appointment(request, doctorusername):
    if request.user.is_authenticated and hasattr(request.user, 'patient'):
        doctor_obj = doctor.objects.get(user__username=doctorusername)
