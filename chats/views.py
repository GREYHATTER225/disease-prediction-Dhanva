...
def messages(request):
    if request.method == "GET":
        ...
        data = [{'sender': m.sender.username, 'message': m.message, 'created': m.created.strftime('%H:%M'), 'is_user': m.sender == request.user} for m in c]
        return JsonResponse({ 'chat': data })
    else:
        return JsonResponse({ 'status': 'error', 'msg': 'GET required' })
